...
**反面案例（同一工具、另一次实测）**：d64 在 120 万字符的池子上只练 2 分钟
（约 3.6 个 epoch，两个副本都严重过拟合），权重平均反而比最好的单副本**差 166.8%**。
这符合已知规律：权重平均要求各副本仍处在同一个损失盆地，过拟合到不同解之后
平均会互相破坏。所以流水线的设计是**先测量再生产**：`merge_replicas.py` 同时评估
每个副本与平均值，`collect_ci_reports.py` 只把最好的那个归档 —— 平均只是候选之一。
